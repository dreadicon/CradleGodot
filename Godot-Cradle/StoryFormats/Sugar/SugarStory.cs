...
{
	public abstract partial class SugarStory: Story
	{
		protected RandomNumberGenerator _rand = new RandomNumberGenerator();
		...
		{
			return vars[_rand.RandiRange(0, vars.Length)];
		}
		...
		{
			return _rand.RandiRange(min, max + 1);
		}
		...
		{
			return _rand.RandfRange((float)min, (float)max);
		}
		...
		protected int time()
		{
			return (int)Math.Round(this.PassageTime * 1000);
		}
