...
namespace Cradle
{
	...
	public abstract partial class Story: Node
	{
		public bool AutoPlay = true;
		public string StartPassage = "Start";
		public List<Node> AdditionalCues;
		...
		public float PassageTime { get { return _timeAccumulated + (Time.GetTicksMsec() - _timeChangedToPlay); } }
		...
		*/
		...
		public void Pause()
		{
			...
			this.State = StoryState.Paused;
			_timeAccumulated = Time.GetTicksMsec() - _timeChangedToPlay;
		}
		...
		void Enter(string passageName)
		{
			...
			_timeAccumulated = 0;
			_timeChangedToPlay = Time.GetTicksMsec();
		...
		void CueInvoke(Cue cue, object[] args)
		{
			...
			{
				this.Warning($"The cue '{cue.method.Name}' doesn't have the right parameters so it is being ignored.");
				return;
			}

			if (result is IEnumerator)
				Timing.RunCoroutine(((IEnumerator<double>)result).CancelWith(this));
		}
		...
		Node[] CueGetTargets()
		{
			if (_cueTargets == null)
			{
				...
				{
					cueTargets = new Node[this.AdditionalCues.Count + 1];
					cueTargets[0] = this;
					this.AdditionalCues.CopyTo(cueTargets, 1);
				}
				else
					cueTargets = new Node[] { this };

				// Get all types
				var sources = new List<Node>();
				for (int i = 0; i < cueTargets.Length; i++)
					sources.AddRange(cueTargets[i].GetChildren<Node>()); // WARNING: this may grab more nodes than it should, as it was originally GetComponents
				...
				for (int i = 0; i < targets.Length; i++)
				{
					...
					foreach (MethodInfo method in methodsFound)
					{
						...
						if (allowCoroutines)
						{
							...
							{
								this.Warning(targetType.Name + "." + methodName + " must return void or IEnumerator in order to be used as a cue.");
								continue;
							}
						}
						else
						{
							...
							{
								this.Warning(targetType.Name + "." + methodName + " must return void in order to be used as a cue.");
								continue;
							}
						}
